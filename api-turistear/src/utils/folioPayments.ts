...
import { affiliateOperators, folioLines, folioPayments, folioPaymentAllocations } from '../db/schema'
import {
  type LineAllocation,
  orderForCascade,
  prorateByWeight,
  splitRows,
} from './folioAllocations'
...
interface PaymentRowInput {
  ...
  // US-LG09 — pre-generated by a caller that attaches allocation rows to this payment in the
  // same batch (the id must exist before the insert statement is built). Omit where nothing
  // references the row.
  id?: string
}
...
// US-LG09 — build the allocation INSERTs that ride a payment/refund row's own batch (rule 1). The
// caller pre-generates the parent row's id and signs the amounts here: +1 for a payment, −1 for a
// refund, so an allocation always carries its parent's sign.
export const allocationRows = (
  db: Db,
  input: {
    organizationId: string
    paymentId: string
    allocations: LineAllocation[] // positive magnitudes (folioAllocations.ts output)
    sign: 1 | -1
    createdAt?: Date
  },
) =>
  input.allocations.map((a) =>
    db.insert(folioPaymentAllocations).values({
      id: crypto.randomUUID(),
      organizationId: input.organizationId,
      paymentId: input.paymentId,
      folioLineId: a.folioLineId,
      amount: input.sign * a.amount,
      ...(input.createdAt ? { createdAt: input.createdAt } : {}),
    }),
  )
...
export const buildCancellationReversal = async (
  ...
  // US-LG09 — the refund rows' negative allocations: each line gives back money in proportion to
  // what it holds (prorateByWeight over the net allocated sums, the same largest-remainder rule as
  // the method buckets above), laid over the per-method refund rows chronologically (splitRows).
  // A folio whose payments carry no allocations yet cannot be split — the shadow is skipped rather
  // than blocking a CANCELLATION on bookkeeping (F1 is a verified shadow; migration 0062's
  // backfill makes this branch unreachable for every pre-feature folio).
  const lineRows = await db
    .select({
      ...
      // The correlation names the outer column RAW (`folio_lines.id`) — the same trick as
      // displayMethodSql above: an interpolated column reference resolves in the subquery's own
      // scope and silently correlates to nothing.
    ...
    .where(
      and(
        eq(folioLines.organizationId, input.organizationId),
  ...
  const shareTotal = lineShares.reduce((s, x) => s + x.amount, 0)
  const perRowAllocations =
  ...
  for (const [i, { method, amount }] of moneyToReverse.entries()) {
    ...
    rows.push(
      ...
      ...allocationRows(db, {
        organizationId: input.organizationId,
        paymentId: refundId,
        allocations: perRowAllocations[i],
        sign: -1,
        createdAt: input.at,
      }),
    )
