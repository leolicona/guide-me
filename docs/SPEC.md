...
- **US-A57** — As an admin, on **Step 3 — Invitations** I want to invite the affiliate's **manager** by email so that, on *Finalizar*, the company + commissions are saved and a Resend magic-link invitation goes to that address (US-AF01). Adding the email is **optional** — I can invite later. **At most ONE credentialed affiliate (the manager) per company** (docs/affiliate-operators/spec.md D13): extra sellers are added by the manager as **PIN operators** (US-AF10), not as more logins — so Step 3 takes a single email, and `inviteAffiliate` / `completeInvite` refuse a second seat (`409 AFFILIATE_MANAGER_EXISTS`). *(Orchestrates US-A49.)*
...
> `docs/affiliate-operators/spec.md`.
