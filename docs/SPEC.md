...
| `corporate` | B2B partner (hotel / travel agency): a self-service portal to book groups **on credit** (QRs issued while unpaid); sees only their own partner's folios and balance | Email + password (admin-invited, verification via Resend magic link) |
...
- **US-A46** — As an admin, I want to configure the **booking policy** for my organization — the **minimum deposit percentage** (`booking_min_down_payment_pct`, 0–100), the **hold window** (`booking_hold_days`, ≥ 1) after which an unsettled apartado auto-cancels and releases its spots, and the **same-day buffer** (`same_day_buffer_minutes`) for tours sold the same day — so apartados match my business rules. Org-scoped; takes effect for **new** bookings only (existing bookings keep their snapshotted expiry). *(New; feeds the US-AG07.1 resolver — the deposit floor and the auto-expiry sweep.)* — `docs/bookings/bookings-down-payments.spec.md`
...
- **US-AG35** — As an agent, I want the `/pos` Date filter to be a **quick-day strip** — **`HOY`** plus the **next two days** (e.g. `HOY · SÁB 14 · DOM 15`), then a **calendar button** — and, on tapping the calendar, a **Bottom Sheet** with a **month grid of square day chips** that marks the **available days of the month** (sellable days emphasized + selectable, past/empty days disabled) and a **month navigation** control, so I pick any selling day in one or two taps while the catalog still **defaults to today**. Picking a day sets the shared `selectedDate` (US-AG30) and closes the sheet; a lightweight `GET /api/pos/availability/days` (org-scoped) feeds the day marks. *(Refines US-AG30's Date control — replaces the interim 14-pill strip + native picker; reuses the US-AG31 Bottom Sheet pattern. Adds one lightweight read; no migration.)* — `docs/pos/date-filter-calendar-sheet.spec.md`
...
### Corporate Partner (B2B Self-Service — Credit Bookings)

> A B2B surface for commercial partners (hotels, travel agencies) who send tourist groups.
> **Admin-invited** `corporate` users transact on a tailored portal, booking on **credit**:
> the group receives scannable QRs **even with a pending balance**, which the admin settles at a
> later accounting cut-off. Each partner is a separate entity carrying a **credit limit** and
> **per-service negotiated net rates**, with one or more concierge logins. Realizes the **deferred
> B2B phase** of Bookings/Down-payments (`docs/bookings/bookings-down-payments.spec.md` §1.2).
> Full spec: `docs/b2b-corporate/b2b-corporate.spec.md`.

- **US-CORP.1** — As a corporate partner, I want my own login to a tailored portal so I can check availability and place group orders without phoning the operator. Adds the `corporate` role; the user is **admin-invited** inside the operator's organization and linked to one `corporate_partner`. On login they land on `/corporate` (not the agent POS / admin panel), seeing only a catalog-with-availability and a checkout — and only **their own partner's** data (cross-partner / cross-org → `404`). — `docs/b2b-corporate/b2b-corporate.spec.md`
- **US-CORP.2** — As a corporate partner, I want to place an order paying a deposit, or nothing upfront, so I secure the group's spots under our post-payment agreement. The folio is created `status:'booking'` with `scan_allowed_unpaid=true`, spots decremented immediately, `commission_amount=0`, and **no `booking_expires_at`** (credit bookings never auto-expire). `down_payment` is optional (`0 ≤ deposit ≤ total`) and **bypasses** the org minimum-deposit floor. A **per-partner credit limit** is enforced **before** the decrement: `outstanding + (total − deposit) > credit_limit` → `409 CREDIT_LIMIT_EXCEEDED`; a suspended partner → `403`. — `docs/b2b-corporate/b2b-corporate.spec.md`
- **US-CORP.3** — As an administrator / scanner operator, I want the group to board on tour day even with a pending balance, so the experience stays smooth and we collect at the cut-off. A credit folio **mints per-line QR + portal token at creation** (not deferred to settle), and the scanner is **narrowly relaxed** to admit `status==='paid' OR (status==='booking' AND scan_allowed_unpaid===true)`. The flag is scoped to credit folios only — a B2C `booking` still reads `NOT_PAID`, and a `cancelled` folio always reads `CANCELLED`. The ticket email is addressed to the partner contact for group distribution. — `docs/b2b-corporate/b2b-corporate.spec.md`
- **US-CORP.4** — As an administrator, I want to see corporate bookings with a pending balance — especially tours already run — so I manage collections and mark folios `paid` once paid. A **cobranza dashboard** lists org-wide credit folios with `partner_name`, `pending_balance`, `tour_date`, and a **`past_event` aging flag**, sorted past-event-first. Settlement is **admin-only** and reuses the existing **one-shot settle** (`amount_paid→total`, `settled_at/by`), made **QR-idempotent** (QRs already exist → no re-mint); a `corporate` caller settling → `403`. Settling frees the partner's credit headroom. — `docs/b2b-corporate/b2b-corporate.spec.md`
- **US-CORP.5** — As an administrator, I want to register corporate partners, set their credit limit and negotiated prices, and invite their users, so the commercial agreement is enforced automatically. Manage a `corporate_partner` (name, contacts, `credit_limit`, status); bulk-upsert **per-service net rates** (a removed rate reverts to public price); invite `corporate` logins linked to the partner (reuses the agent-invite flow). Suspending a partner blocks **new** credit bookings while leaving existing folios and QRs intact. — `docs/b2b-corporate/b2b-corporate.spec.md`
- **US-CORP.6** — As a corporate partner, I want my negotiated prices reflected automatically, so the order total matches our agreement with no manual adjustment. At cart time the server resolves each line as `partner_rate.net_price ?? service.public_price`; the `total` is computed **server-side** (the client never sends prices) and the net price is **snapshotted** onto the `folio_line`, so a later rate change never retro-shifts an existing folio. — `docs/b2b-corporate/b2b-corporate.spec.md`
...
- [ ] **B2B Corporate Partners & Credit Bookings** *(US-CORP.1–.6)* - *Self-service portal for commercial partners (hotels / agencies). Admin-invited `corporate` users book groups on **credit** via a separate `corporate_partners` entity (per-partner **credit limit** + per-service negotiated **net rates**, many concierge logins). Folios are created `status:'booking'` with `scan_allowed_unpaid=true` and **QRs minted at creation**, so the group boards even while unpaid — a narrow scanner exemption admits `booking + scan_allowed_unpaid` (B2C bookings still `NOT_PAID`). Credit folios are excluded from the auto-expiry sweep. Admins collect via a **cobranza dashboard** (past-event aging) and an **admin-only, QR-idempotent one-shot settle**. Realizes the deferred B2B phase of Bookings/Down-payments. Additive migration: 2 tables + the `corporate` role + folio flags (`scan_allowed_unpaid`, `corporate_partner_id`).* — `docs/b2b-corporate/b2b-corporate.spec.md`
- [ ] **POS Date Filter — Quick-Day Strip + Calendar Bottom Sheet** *(US-AG35, US-A45)* - *Replaces US-AG30's interim Date control with a quick-day strip (`HOY` + the next two days + a calendar button) and a calendar Bottom Sheet of square day chips that marks the available days of the month (with month navigation), shared identically by agent and admin-seller. The catalog still defaults to today; picking a day sets the shared `selectedDate`. Adds one lightweight org-scoped read `GET /api/pos/availability/days` (the dates with a sellable slot); no migration.* — `docs/pos/date-filter-calendar-sheet.spec.md`
...
| **Quick-day strip** | The `/pos` Date filter's always-visible row: **`HOY`** + the **next two days** as one-tap pills, then a **calendar button** opening the Calendar Bottom Sheet. Sets the shared `selectedDate` (US-AG30); identical for agent and admin-seller (US-AG35 / US-A45). |
