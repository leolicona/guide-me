...
> property/listing that owns **named, individually-bookable units**; each unit carries its own
> nightly pricing, occupancy, availability controls, and amenities, and is sold for a **date range**
> (multi-night stay) rather than a per-day slot. This is a new inventory primitive **beside** slots —
> tours are untouched. Full spec: `docs/lodging/accommodation-stays.spec.md`.

- **US-A59** — As an admin, I want to define a **lodging service** and add **named units** under it (e.g. "Cabaña 1", "Suite Vista"), each with a **unit type** label, **number of beds**, **base occupancy**, and **maximum capacity**, so my property's inventory is modeled as distinct bookable rooms/cabins. Units are children of the `lodging` service (`accommodation_units`), soft-deactivated never hard-deleted (folio history), and the property's catalog card shows *"Desde $X / noche"* (the min unit rate). *(D1/D2 — named individual units, per-unit config.)* — `docs/lodging/accommodation-stays.spec.md`
- **US-A60** — As an admin, I want to set each unit's **rate rules** — a **base price per night**, an optional **weekend rate**, **seasonal overrides** (named date ranges with their own nightly rate), and a **cost per extra person per night** above the base occupancy — so pricing reflects demand and party size. Per night the engine resolves **seasonal > weekend > base**; weekend days are org-configurable (default Fri+Sat). *(D3 — pricing precedence + extra-person surcharge.)* — `docs/lodging/accommodation-stays.spec.md`
- **US-A61** — As an admin, I want each unit's **availability controls** — a **block-out calendar** (dates the unit can't be sold), a **minimum stay** (min nights), and **check-in / check-out times** — so I protect maintenance/owner days and enforce my stay rules. Availability uses **standard hotel turnover** (a stay occupies nights `[check-in … check-out)`; the check-out day is reusable same-day). *(D4/D5/D6.)* — `docs/lodging/accommodation-stays.spec.md`
- **US-A62** — As an admin, I want to tag each unit with **amenities** (WiFi, parking, kitchen, A/C, pool, …) from a closed set, so agents and tourists see what each unit offers. Stored as amenity enum keys on the unit; labels live on the frontend. *(D6.)* — `docs/lodging/accommodation-stays.spec.md`
...
> Agents **and affiliates** sell a multi-night stay by choosing a date range and a named unit
> (US-A59–A63), through the same cart → folio → checkout flow used for tours, with apartado/deposit
> support. Full spec: `docs/lodging/accommodation-stays.spec.md`.

- **US-AG36** — As an agent/affiliate, I want to sell a stay **range-first**: I pick a **check-in and check-out date** (and the number of guests), and the POS shows me **only the units available for the whole range** with the **computed total** (nights × nightly rate, including weekend/seasonal rates and the extra-person surcharge), so a tourist can stay more than one day. Powered by `GET /api/pos/lodging/:serviceId/availability?check_in=&check_out=&guests=`; units that fail min-stay, capacity, a block-out, or an overlapping reservation are hidden. Adding a unit puts a **stay line** in the cart. *(D7/D8 — reuses the cart/folio flow.)* — `docs/lodging/accommodation-stays.spec.md`
- **US-AG37** — As an agent/affiliate, I want to also sell a stay **unit-first**: I open a specific unit, see its **month availability calendar** (free / blocked / booked days at that unit's rate), and select the range on it, so I can satisfy a tourist who wants a particular cabin. Powered by `GET /api/pos/lodging/units/:unitId/calendar?from=&to=`. *(D8 — both entry points.)* — `docs/lodging/accommodation-stays.spec.md`
- **US-AG38** — As an agent/affiliate, I want a stay to **check out like any other sale** — the line shows unit · `Sáb 10 → Mar 13 · 3 noches` · guests · total (with a per-night breakdown), and the **adaptive amount-driven checkout** (US-AG07.2) lets me take **full payment or a deposit (apartado)**, holding the unit for the dates immediately. On confirm the server re-quotes and snapshots the price and inserts the reservation under an **atomic overlap guard** (a concurrent double-booking gets `409 UNIT_UNAVAILABLE`); cancel/expiry/reactivate release or re-hold the dates. Commission follows US-A12 (percent on the stay amount; fixed counts per stay line). *(D7/D9; builds on US-AG07/AG07.2/AG07.4/AG07.5 and US-AG11.)* — `docs/lodging/accommodation-stays.spec.md`
...
- **Accommodation uses a date-range inventory, not slots** (US-A59–A63 / US-AG36–38 — `docs/lodging/accommodation-stays.spec.md`): a `lodging` service owns **named units**; a stay occupies a unit for the nights `[check_in … check_out)` (**standard hotel turnover** — the check-out day is reusable same-day). A unit is available for a range only when no **active reservation** and no **block-out** overlaps it, the party fits `max_capacity`, and `nights ≥ min_nights`. Confirming a stay inserts a reservation under an **atomic overlap guard** (concurrent double-booking → `409 UNIT_UNAVAILABLE`); cancel/expiry releases it. Lodging services never use slots/schedules.
...
- **Accommodation is priced per night** (US-A60 — `docs/lodging/accommodation-stays.spec.md`): a stay total = Σ over each night of the unit's nightly rate + an extra-person surcharge above the base occupancy. Per night the rate resolves **seasonal override > weekend rate > base rate** (weekend days org-configurable, default Fri+Sat). Per-night manual discounting is out of scope for v1. Paid-stay cancellations follow a **structured free-window + penalty %** (US-A63); apartado deposits on a stay stay non-refundable (US-AG07.4).
