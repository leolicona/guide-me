...
|---|---|
| **`redeemed_count` is increment-only and there is no un-redeem endpoint** — one mis-scan is permanent, and `utils/cancellationPolicy.ts` **D7** makes any line with `redeemedCount > 0` retain 100 % and refund nothing. `all_passes` does not worsen this (the ladder already fires at the first pass), but it makes a mis-scan cost the whole party. | Out of scope here — logged as its own defect. The smallest fix is an admin-only `POST /api/tickets/:lineId/unredeem` with an audit note; it needs its own decision about who may forgive a boarding. |
