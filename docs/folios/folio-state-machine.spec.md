...
|---|---|
| Should a `no_show` folio still be cancellable by an admin? | ~~Today it can be, and the ladder gives it 0% — so the outcome is already right. Blocking it would need one guard in `cancelFolio`; leave it until someone reports being confused.~~ **Answered (2026-08-07, the detail's action ladder):** the UI hides the cancel once **every** dated line has departed — the folio is a countable fact (its fulfilment reading), not a cancellable sale: no seats to release, terminal-tier money, and cancelling would only falsify the wasted-seat record. A mixed folio (a departed line + a future one) keeps the cancel: the future line still holds seats. The API stays permissive. |
