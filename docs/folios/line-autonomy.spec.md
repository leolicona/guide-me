...
| **D6** | **Refund debt lives on the line; the PIN stays on the folio.** `refund_status`/`refund_amount` move to `folio_lines`. `confirmRefund` settles all of the folio's pending line-debts present at the handshake. *(Amended in build — see rule 8: the ledger rows land at cancellation, not at the confirm.)* | Two lines cancelled on different dates are two debts; folding them into one folio number is the information collapse this corpus keeps paying for. The PIN authenticates the **person** at the counter, not an amount — one tourist, one PIN. Per-line PINs would be friction with no threat model behind it. |
...
7. The expiry sweep cancels exactly the lines whose own clock fired (D5); a folio may end half
   expired, half alive.
8. `confirmRefund` (PIN or audited override) settles the folio's pending line-debts, then
   **rotates the PIN** and resets attempts (D7). *(Amended in build, F2: the signed refund rows +
   their allocations land at CANCELLATION — the ledger reverses when the obligation is born,
   which is the paid-ledger's shipped semantics — so the confirm writes no second ledger row; as
   originally worded it would have double-counted the reversal. The confirm records only the
   physical hand-back: line `refund_status` flips, the folio rolls up, the PIN rotates.)*
...
- [x] `folio_payment_allocations` + Drizzle schema; `confirmSale`/`settleBooking` write
      allocations in-batch (S-1…S-4)
- [x] Backfill: every existing folio allocated, idempotent, `backfilled = 1` (S-15); the SQL
      cascade verified against the TypeScript oracle (S-16)
- [x] Conservation invariants asserted over the whole DB post-migration AND in write-path
      tests (scope boundary 3)
- [x] `seed:local` produces mixed fixtures
- [x] Scope-boundary tests pass unedited

**F2 — US-A22** *(migration 0063)*
- [x] Line cancel endpoint through the single pricing commit (S-5, S-6)
- [x] Backfill: cancelled folios' lines arrive cancelled; refund debt distributed pro-rata
- [x] Per-line refund debt + rotating PIN (S-7); portal shows line-level outcomes
- [x] `folio_events`/`notifications` line-scoped (D13); timeline renders line names
- [x] Cross-org (S-14)

**F3 — the gesture** *(migration 0064)*
- [x] Per-line settle + "Liquidar todo" (S-8, S-10); per-line clock + sweep (S-9)
- [x] Backfill: existing apartada lines copy the folio's snapshotted clock (S-17)
- [x] Reminder message itemizes pending lines (folio-level send, D12) — *the template's
      `{itinerary}`/`{pending_balance}` already resolve folio-wide; no change was needed*

**F4 — cleanup**
- [x] Readers migrated PR by PR (gates, lists, facets, search, portal) — every reader derives
      from the lines (D11; S-11…S-13). *The derived field is not deleted: it now IS the field,
