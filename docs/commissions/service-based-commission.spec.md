...
| D3 | **Fixed is per spot, capped by the price floor** | A fixed commission scales with `quantity` and must satisfy `commission_value ≤ minimum_price`. **Exception:** `lodging` services are exempt — they have no price floor (units price per night), so a fixed base/override is uncapped (see `docs/lodging/accommodation-stays.spec.md` § Commission waterfall). | Per-seat is the operator's mental model and scales like percent. The cap guarantees commission never exceeds the revenue of even a maximally-discounted pass — structurally removing the "discount to the floor, still earn full commission > margin" trap (US-AG06 interaction). |
