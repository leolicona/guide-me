...
| **D6** *(revised twice in build)* | The grid is **visible on open** and collapses once a day is picked; «Cambiar» reopens it. It was briefly collapsed-on-open as a fix for the sheet overflowing 85vh — D18 removed the header weight that actually caused that, so the collapse went back to serving only its original purpose (making room for zones and extras). | The sheet caps at 85vh (~717px at 390×844). Grid + pager + chips + zone + extras + footer does not fit, and the one control that must never be scrolled away is *Agregar al carrito*. **The build proved opening expanded had the same fault the collapse was meant to fix** — every sale began with a scroll, reported from use. D1 still holds: the grid is the *only* way to pick a date. What changed is that it is not the first thing between the seller and a sale they were already set up to make — the anchor (D5) has, by construction, already chosen a sellable day. |
| **D15** *(added in build)* | The month read is **deferred until the grid is expanded**, and `DateRangeCalendar` gets a `compact` geometry (40px rows, 4px gaps) used only by this host. | A sheet opened to sell today's 15:00 should not spend a round trip painting a month nobody asked to see — D2's argument was that the month is *cheap*, not that it is free. The compact grid saves ~90px over six rows so the expanded state also fits without scrolling; the full-page and lodging hosts keep the roomier default. |
| **D18** *(added in build)* | The sheet adopts the **lodging sheet's structure**: name → calendar → day pager + times → Personas → footer. The service **description**, the **price line**, the *«Elige una fecha y horario»* label and the calendar **legend** are removed. | Requested from use, and it resolves D6's revision at the root rather than working around it. The header was carrying a description and a price the seller had just read on the card, plus the Personas stepper — ~150px spent *above* the calendar, which is what pushed the sheet past 85vh and forced the collapse-on-open. With them gone the grid leads the sheet the way it leads Reagendar and hospedaje. The removed label named what the calendar visibly is; the legend spent two lines explaining a paint that already reads (dot = sellable, tint = full, flat = does not run). **D8's distinction survives in the cells, not in a caption.** |
| **D19** *(added in build)* | Personas moves **below** the calendar. | It is the lodging order, and it is the better one here: the group re-classifies the grid (D4/D12), so putting it after means the seller watches the calendar answer *change to their input* instead of being asked to commit to a group before seeing a single date. |
| **D17** *(added in build)* | The ◀ ▶ pager steps to the next day **with room**, not to the next calendar day. Its month is always fetched; the neighbour months load only when the current one cannot answer. | Reported from use. A service running Fri/Sat/Sun made crossing a week four taps of *"no sale ese día"* — the flat-list problem this feature exists to end, re-created inside the sheet. `RescheduleSheet` never had it because its axis was only ever days with room. This **narrows D15**: the grid's month stays deferred, but the pager's is load-bearing, so a sheet open still costs one read. The neighbours matter because a month boundary must not read as "no more departures" — a service whose next run is the 2nd has nothing left in the current month, and disabling ▶ there would hide real inventory. |
| **D16** *(added in build)* | `usePosService` holds the **previous day's payload** while a new day loads (`placeholderData`, scoped to the same service), and only the time chips show a skeleton. | Reported from use: each date change flipped `isLoading`, the owner's `{service && …}` gate dropped the panel, and the Bottom Sheet collapsed to a spinner and sprang back — reading as the sheet closing and reopening under the thumb. Reagendar never showed this because it fetches 60 days **once** and filters in the client; D2 traded that payload for a per-day read and inherited its remount. Scoping to the same service matters: `prev` is the last resolved query whatever its key, so an unscoped hold would flash the previous service's name and price on the next card — a worse lie than a spinner. |
...
- [x] `sellableDayAxis` / `nextSellableDay` / `prevSellableDay` + tests — the pager skips gaps,
      dedupes overlapping months, drops stale past days and crosses a month boundary (D17).
