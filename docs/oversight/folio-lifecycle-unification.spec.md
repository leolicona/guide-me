...
| **D21** *(2026-08-08)* | **`FolioWorkActions` is THE pending-action card**: the detail's whole D12 ladder — solicitud → verificación → reembolso → entrega — resolved inside one component, **exactly one rung rendered**. Rung 3 keeps its warning anatomy (a cash obligation must not read like a neutral title); rung 4 is also the card's resting face — it stays for re-sending after delivery, because re-send is a tool that needs a home. | The exclusion used to be three booleans threaded through three blocks of the page, with three different visual treatments (work card / amber `Alert` / loose `SectionCard`) for the same rank of thing. One component, one branch: a future slip **cannot** render two rungs, and a fixed position teaches "the card under the chips is what this folio needs from me". |
| **D22** *(2026-08-08)* | **Single-subject containers.** The header is the CUSTOMER's card: name, tap-to-dial phone + the WhatsApp button, email, state chips. The payment card is the SALE: a "Vendido por {agente} · {fecha}" letterhead (moved from the header), then lines, then totals. | Under one shared border the eye groups: a phone number inside the payment card parsed as sale data, and the seller's name in the header parsed as customer data. Each fact moves to the container whose subject it belongs to — and the header giving up the agent line pays for the room contact takes. |
| **D23** *(2026-08-08)* | **The cancellation's money outcome is recorded in the totals**: `Se devuelve al cliente` (delivery state icon-paired on the row — *Por entregar* / *Entregado*) and `La empresa retiene`, for any cancelled folio with money paid. `refund_status: 'none'` with money paid renders as full retention, said out loud. | The `RefundQuote` shows exactly this pair BEFORE the admin commits; without this, the card afterwards read as if the sale were still whole (`Pagado $3,000` and silence). The quote is the decision's preview, the totals are its permanent record — same vocabulary, nothing to relearn. The WHEN and HOW stay the Historial's. |
| **D24** *(2026-08-08)* | **No banner zone.** A banner exists only if it instructs about the *now*; `ExpiredBookingBanner` retires from both detail surfaces, and its one non-redundant sentence (a returning customer is a NEW sale; the credit applies as manual discount) rides the `Saldo a favor` row it was about. | After D21–D23 every former banner's content has a better-ranked home: past facts in the Historial, state in the chips, money in the totals, work in the pending-action card. What remained of the expiry banner was a state restated (chips already say it) plus one instruction — and an instruction about credit belongs next to the credit. |
...
*(2026-08-08 — D21–D24: the page's anatomy settles as header (customer) → pending-action card
(`FolioWorkActions`, the whole ladder) → Historial (collapsed) → payment card (sale letterhead +
lines + totals with the refund outcome) → action block. The refund `FormSheet` stays with the
page — its success path opens the receipt composer — the card signals via `onConfirmRefund`.
`FolioHistoryDetailPage` mirrors D22/D24: customer line in the header, no banners.)*
