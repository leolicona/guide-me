...
| **D17** | **The rail is computed SURFACE-BLIND; only the verb is capability-filtered.** The petition gains an icon-paired line on the card, so the colour it turns is never alone. | Found by the `/design-review` pass, not by reading the code (BUG-035): deriving the rail from the already-filtered `folioAction` meant hiding the admin *verb* also downgraded the *state*, and a folio whose customer was waiting on an answer rendered GREEN — "nothing needs a human" — on the seller's list. The rail answers *does this folio need a human?*, which is a fact about the folio; the button answers *may I act?*, which is a fact about the reader. Conflating them was safe while one surface existed. **Amends `line-autonomy.spec.md` D14**, whose "colour and verb can never disagree" holds within a surface and cannot hold across two: the seller now legitimately sees an amber rail above a neutral verb, with the line naming why. |
...
**S-15 — A waiting customer is never green (BUG-035)**
Given a folio of the seller's with an open petition
When their list renders it
Then the rail is the attention colour the folio earns — not `ok` — and a line on the card names
the request in words, while the button still offers only a verb the seller may press.
